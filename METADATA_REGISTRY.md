...
Run `npm run registry:generate` after changing the source. CI runs `npm run registry:check` and fails if the generated runtime maps are stale, non-deterministic, or contain duplicate IDs/names. The generated module contains immutable maps plus the `MetadataFieldName` union.
