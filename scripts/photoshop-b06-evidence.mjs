...
const humanReview = JSON.parse(await readFile(join(root, "reports/photoshop-b06-human-review.json"), "utf8"));
...
const report = {
  ...
  humanReview: { required: true, status: humanReview.status, scope: "every B06 writer change and exact Photoshop resource targeting diff", selfReviewIsNotApproval: true, record: "reports/photoshop-b06-human-review.json" },
};
...
const markdownWithReviewStatus = markdown.replace("**pending**", `**${report.humanReview.status}**`);
await writeFile(join(reports, "photoshop-b06-evidence.md"), markdownWithReviewStatus);
