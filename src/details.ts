...
export function deriveImageDetails(result: ParsedMetadataResult, bytes?: Uint8Array, limits?: SecurityLimits, mapOffset?: (offset: number) => number): ImageDetails {
  ...
  if (result.heifSequences !== undefined) {
    ...
    for (const sequence of result.heifSequences) {
      if (sequence.primaryTrackId !== null) raw.primaryImage.push(candidate(`track:${sequence.primaryTrackId}`, `HEIF sequence:${sequence.sourceOffset}`, sequence.sourceOffset, null, sequence.primarySelection === "sole-picture-track" ? "valid" : "conflicting", "Selected only when exactly one picture track is present."));
